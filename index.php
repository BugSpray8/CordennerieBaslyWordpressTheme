...
$wrap_class              = apply_filters( 'basly_filter_index_search_content_classes', 'col-md-8 blog-posts-wrap' );
$alternative_blog_layout = get_theme_mod( 'basly_alternative_blog_layout', 'blog_normal_layout' );
$wrap_posts              = 'flex-row';
if ( Basly_Public::should_enqueue_masonry() === true ) {
	$wrap_posts .= ' post-grid-display';
}

do_action( 'basly_before_index_wrapper' ); ?>
...
<div class="<?php echo basly_layout(); ?>">
	<div class="basly-blogs" data-layout="<?php echo esc_attr( $sidebar_layout ); ?>">
		<div class="container">
			...
			do_action( 'basly_before_index_posts_loop' );
			$paged         = get_query_var( 'paged' ) ? get_query_var( 'paged' ) : 1;
			$posts_to_skip = ( $paged === 1 ) ? apply_filters( 'basly_filter_skipped_posts_in_main_loop', array() ) : array();
			...
			<div class="row">
				...
				<div class="<?php echo esc_attr( $wrap_class ); ?>">
					<?php
					do_action( 'basly_before_index_content' );
					...
					if ( have_posts() ) {
						...
						echo '</div>';
						$basly_pagination_type = get_theme_mod( 'basly_pagination_type', 'number' );
						if ( $basly_pagination_type === 'number' ) {
	...
	<?php do_action( 'basly_after_archive_content' ); ?>
