...
$layout               = apply_filters( 'basly_header_layout', get_theme_mod( 'basly_header_layout', 'default' ) );
...
$header_class = '';
$hide_top_bar = get_theme_mod( 'basly_top_bar_hide', true );
...
<body <?php body_class(); ?>>
	...
	<div class="<?php echo esc_attr( $wrapper_div_classes ); ?>">
		<header class="header <?php echo esc_attr( $header_class ); ?>">
			<?php
			basly_before_header_trigger();
			do_action( 'basly_do_top_bar' );
			do_action( 'basly_do_header' );
			basly_after_header_trigger();
			?>
		</header>
